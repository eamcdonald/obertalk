...
In this talk I want to introduce my work in my pair of papers \href{https://arxiv.org/abs/2601.17794}{arXiv:2601.17794} and \href{https://arxiv.org/abs/2410.13701}{arXiv:2410.13701}, plus some upcoming work.
...
\begin{frame}
    \huge{Section \ref{Intro}: Hypoellipticity (and some puzzles)}
\end{frame}
...
\[
    \Lc_b = (\partial_x-\frac12y\partial_z)^2+(\partial_y+\frac12x\partial_z)+ib\partial_z.
\]
Then:
\begin{itemize}
...
For a long time, people expected something similar would be true for PDE with smooth coefficients. But it's not so: Let
\[
...
\emph{Puzzle 3}: What is so special about $L$?
\end{frame}
...
\begin{frame}
    \Huge{Section \ref{Hormander_section}: H\"ormander's sum of squares theorem}
\end{frame}
...
\begin{frame}
    \Huge{Section \ref{nilpotent_lie_groups_section}: Analysis on graded Lie groups}
\end{frame}
...
\begin{frame}
    \Huge{Section \ref{Carnot_section}: Analysis on Carnot manifolds and the tangent groupoid}
\end{frame}
...
\begin{frame}{Illustrative example: Asteroids}
    ...
    \[
        0 = H^0 < H^1 < H^2=TM.
    \]
    The osculating groups are all the Heisenberg groups. The operator
    \[
        \Xi = X^2+Y^2
    \]
...
\begin{frame}{Connes' trace theorem}
    \begin{theorem}
        Let $M$ be a compact $d$-manifold, and let $T \in \Psi^{-d}(M)$ (a classical pseudodifferential operator of order $-d.$) Then $T$ can be realised as an operator belonging to the weak-trace-class ideal $\Lc_{1,\infty},$ and for any normalised trace $\varphi$ on $\Lc_{1,\infty}$ we have
        \[
            \varphi(T) = \frac1d\mathrm{Res}(T)
        \]
        where $\mathrm{Res}(T)$ is the Wodzicki residue.
    \end{theorem}
\end{frame}
